...
import { UserPlus, Search, User, Phone, Mail, Edit2, X, Users } from 'lucide-react';
...
const Patients = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = getToken();
      if (editingId) {
        const updated = await apiPatch(`/patients/${editingId}`, form, token);
        if (updated._id) {
          setPatients(patients.map(p => p._id === editingId ? updated : p));
          setMessage({ text: 'Patient updated successfully.', type: 'success' });
          cancelEdit();
        } else {
          setMessage({ text: updated.message || 'Failed to update patient.', type: 'error' });
        }
      } else {
      ...
      setMessage({ text: 'Operation failed. Check connection.', type: 'error' });
    }
  };
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <button className={`btn ${showForm ? 'btn-ghost' : 'btn-primary'}`} onClick={() => showForm ? cancelEdit() : setShowForm(true)}>
          {showForm ? <><X size={18} /> Cancel</> : <><UserPlus size={18} /> New Patient</>}
        </button>
      </div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: '1.5rem', border: editingId ? '1px solid var(--primary)' : '1px solid var(--border)' }}>
          <h3 style={{ marginBottom: '1.25rem' }}>{editingId ? 'Edit Patient Details' : 'Register New Patient'}</h3>
          <form onSubmit={handleSubmit} className="form-grid">
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button type="submit" className="btn btn-primary">{editingId ? 'Update Patient' : 'Register Patient'}</button>
              ...
              <tbody>
                {filtered.map(p => (
                  <tr key={p._id}>
                    ...
                    <td>{p.email || '—'}</td>
                    <td>
                      <button className="btn btn-ghost" onClick={() => handleEdit(p)} title="Edit Patient" style={{ padding: '0.4rem' }}>
                        <Edit2 size={16} />
                      </button>
                    </td>
                  </tr>
